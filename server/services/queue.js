export class QueueManager {
  ...
  initialize() {
    ...
    try {
        ...
        const tableInfo = this.db.pragma("table_info(jobs)");
        const hasMessageId = tableInfo.some(col => col.name === 'message_id');
        
        if (!hasMessageId) {
            this.db.exec(`ALTER TABLE jobs ADD COLUMN message_id text;`);
        }
        // Always ensure index exists after column is guaranteed
        this.db.exec(`CREATE INDEX IF NOT EXISTS idx_jobs_message_id ON jobs(message_id);`);
    } catch (e) {
    ...
    this.stmtUpdateMessageId = this.db.prepare(`update jobs set message_id=@message_id where id=@id`);
    this.stmtUpdateStatusByMessageId = this.db.prepare(`update jobs set status=@status, updated_at=datetime('now') where message_id=@message_id`);
    ...
    this.stmtCampaignTotals = this.db.prepare(`
      select campaign_id as id,
             ...
             count(*) as total
      from jobs group by campaign_id order by max(created_at) desc limit @limit offset @offset
    `);
    this.stmtCampaignCount = this.db.prepare(`select count(distinct campaign_id) as c from jobs`);
    ...
    this.stmtRecentErrorsBySession = this.db.prepare(`
      ...
      limit @limit
    `);
    this.stmtRecentSentBySession = this.db.prepare(`
      select id, campaign_id, session_id, phone, message, status, updated_at, created_at
      from jobs where session_id=@session_id and status='sent'
  ...
  getNextJob(sessionId) {
    const job = this.stmtNextJob.get({ session_id: sessionId });
    if (job) {
      // Immediately mark as processing/locked to avoid race conditions if we had multiple workers (not the case here but good practice)
      // For now, we just return it. The worker will mark it as sent or failed.
    }
    return job || null;
  }

  completeJob(id) {
    this.markJob(id, 'sent');
  }

  failJob(id, error) {
    this.markJob(id, 'failed', error);
  }
  ...
  updateJobMessageId(id, messageId) {
    this.stmtUpdateMessageId.run({ id, message_id: messageId });
  }

  updateJobStatusByMessageId(messageId, status) {
    this.stmtUpdateStatusByMessageId.run({ message_id: messageId, status });
  }
  ...
  listCampaigns(limit = 20, offset = 0) {
    ...
    const totalCampaigns = this.stmtCampaignCount.get().c;
    return {
  ...
  getRecentSentBySession(sessionId, limit = 1000) {
    return this.stmtRecentSentBySession.all({ session_id: sessionId, limit });
  }
